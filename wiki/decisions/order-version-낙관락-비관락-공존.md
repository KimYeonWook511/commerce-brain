...
> [!note] 결제 쪽 낙관 락과의 관계
> 주문과 결제가 낙관 락을 각자 도입했고 동기가 갈린다 — 주문은 만료/결제 race를 잡는 것이고, 결제는 상태 전이가 여러 경로에서 일어나는 것이 동기다([[payment-낙관적-락-도입-왜-비관-아님]]). 부분취소가 들어오며 "주문의 비관 락을 낙관으로 전환할지"를 다시 판단했고 **주문 행 비관 락을 그대로 유지**하기로 했다([[부분취소-동시성-주문행-단일잠금과-캐시겹-미도입]]).
