...
> [!note] 진화 (2026-08) — 이 3계층 구조는 유효하고, 그 위에 셋이 얹혔다
> 2026-08 결제·환불 재설계에서 이 노트를 14곳이 인용하는데 구조 자체는 안 바뀌었다. 더해진 것만 적는다.
> - **낙관 락이 어느 행에 붙는지가 갈렸다** — 자식(환불)이 자기 락을 갖고 부모 버전은 불변식이 바뀔 때만 오른다([[자식-환불-자기-낙관락-부모버전은-불변식이-바뀔때만]]). 안 그러면 배치가 자식을 만질 때마다 부모 버전이 올라 회원 요청을 밀어낸다.
> - **재시도 소진 시 응답 표현이 아직 경로마다 다르다** — 결제 시작 경로는 기술 예외 원형이 끝단까지 가서 공통 409로, 주문 취소 경로는 도메인 예외로 번역해 그 도메인 코드로 나간다([[무결성위반-도메인예외-번역을-제약이름으로-가른다]]의 미해결).
> - **이 노트가 낙관 락 단독 방어선이 된 자리가 생겼다** — 전파 속성을 바꾸면서 요청 스코프 영속성 컨텍스트가 1차 캐시를 남겨, 전에는 "매번 DB를 다시 읽는다"와 이중이던 것이 낙관 락 단독이 됐다([[애너테이션-잔재-판별과-전파변경의-osiv-파급]]).
> - 재시도 루프가 트랜잭션 밖에 있다는 이 노트의 배치가 외부 조언에 반박하는 근거로 쓰였다([[예외타입-실측과-테스트가-명세를-넘어-단언하는-것]]).
