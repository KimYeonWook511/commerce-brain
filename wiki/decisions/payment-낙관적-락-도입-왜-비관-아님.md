...
> [!note] 예약해둔 재판단이 닫혔다 (2026-08)
> 이 노트가 "부분취소(합산 검증)가 들어오면 비관이 재정당화될 수 있다 / 주문의 비관 락을 낙관으로 전환할지는 그때 다시 판단한다"로 미뤄둔 것이 정리됐다. **주문 행 비관 락은 그대로 유지하고, 품목 행까지 잠그지는 않기로 했다**([[부분취소-동시성-주문행-단일잠금과-캐시겹-미도입]]). 다만 새 모델에서 한도 검증이 합산이 아니라 **누적액 컬럼 하나 읽기**로 바뀌어 결제 쪽에서는 낙관 락만으로 성립한다([[환불-독립-aggregate-한도판정은-결제가-누적액-컬럼]]).
> 낙관 락 배치도 한 번 더 갈렸다 — **자식(환불)이 자기 낙관 락을 갖고 부모 버전은 불변식이 바뀔 때만 오른다**([[자식-환불-자기-낙관락-부모버전은-불변식이-바뀔때만]]).
