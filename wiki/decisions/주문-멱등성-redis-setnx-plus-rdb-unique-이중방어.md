...
tags: [order, idempotency, redis, concurrency, unique-constraint, race-condition]
created: 2026-05-29
updated: 2026-08-17
superseded_by: null
sources:
...
>
> **#171(PROCESSING 동시 요청이 안전망 500을 받음)이 결제 쪽에서 답을 얻었다** (2026-08) — 진 쪽에게 성공도 서버 오류도 아닌 **"같은 요청이 처리 중" 전용 응답**을 준다([[동시도착은-선점층이-받고-처리중-전용응답]]). 커밋 전이라 성공으로 답할 수 없다는 것이 그 근거다.
>
> 그리고 이 노트의 **"(회원, 멱등키) 복합 unique"가 다른 흐름의 일관성 근거로 인용됐다가 정정됐다.** 취소는 회원 단위가 아니라 주문 단위가 맞았다 — **주문을 만들 때는 아직 주문이 없어 회원 말고 짝지을 것이 없었을 뿐이고, 선택이 아니라 제약이었다**([[취소요청키-유일범위-주문단위-와-같은키-다른내용-거부]]). 유일 범위를 조회 범위와 맞춘다는 일반 규칙은 [[멱등키-세-값-분리와-요청멱등키는-호출자가-발급]].
