...
> [!warning] 이 결정의 "단일 tx"가 코드에서는 성립하지 않고 있었다 (2026-08 정정)
> 아래가 단정한 "환불 의도 저장 + 주문 취소 + 재고 복구를 한 트랜잭션에서 원자적으로 커밋한다"가 **구현과 어긋나 있었다.** 취소 사건 생성 서비스에 남은 전파 속성 잔재 하나가 바깥 트랜잭션을 중단시켜, 그 사건이 **먼저 별도 커밋**되고 있었다. 고아 취소 사건이 남으면 대사가 그것을 집어 실제로 환불을 집행한다 — **환불은 나갔고 주문은 결제완료로 남고 재고는 복구되지 않고 아무도 모른다.**
> 결론은 **구현을 이 결정에 맞추는 것**이었다([[취소접수-트랜잭션경계-구현을-결정에-맞춘다-전파속성-잔재]]). 즉 이 노트의 결정 자체는 옳았고 코드가 어긋나 있었다. 그 판별 방법은 [[애너테이션-잔재-판별과-전파변경의-osiv-파급]].

> [!note] 진화 (2026-08) — 대사 정책과 부분취소 확장
> - **"대사의 판단 근거"가 바뀌었다.** 아래는 "요청됨 상태의 취소 사건이 있다 → 환불해도 된다"에 기대는데, 그 전제가 시스템 전체에서 참이 아니다. 새 모델은 **환불이 만들어지는 자리를 정당한 조건과 같은 트랜잭션으로 묶어 확인 자체가 필요 없게** 했다([[회수배치가-주문상태를-묻지-않는다-확인대신-전제를-지킨다]]).
> - **확정적 실패(FAILED)를 통지로만 종착시킨다는 결정이 뒤집혔다.** 환불에는 자동으로 도달하는 실패 종착이 없다 — **환불 실패는 정상 종료가 아니기 때문이다**([[승인은-다시-물어-확정-환불에는-실패-종착이-없다]]).
> - **주문 행 단일 잠금 결정은 그대로 유효하고**, 아래가 우려한 "품목 행에도 잠금이 생기면 교착이 난다"가 부분취소 설계에서 그대로 기각 근거로 쓰였다([[부분취소-동시성-주문행-단일잠금과-캐시겹-미도입]]).
> - **부분취소는 [[부분취소-스코프-배송전-품목수량-기반-금액입력-기각]]에서 열렸다.** 아래가 예고한 "취소 요청 키 + 한도 검증" 방향은 맞았지만 잔액의 정본은 금액이 아니라 수량으로 갔다([[부분취소-잔액-정본-수량기준-상태무관-누적]]).
