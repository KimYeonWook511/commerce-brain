...
- RTR 이 *다른 기기 로그인 시 이전 기기 세션 자동 무효화*는 이미 해주므로(앞의 JWT+Redis+RTR 하이브리드 설계), logout API 의 *추가 가치*는 사용자의 *의식적 종료 의사 표현*에 한정된다 — 자동/자연 발생이 아닌 *의도적 트리거*만 logout 의 영역.
